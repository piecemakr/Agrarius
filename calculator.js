...
units.addEventListener('change', function handleChange(event) {
	if (event.target.value === 'Hectares') {
		$('.hectares').css('display','block');
		$('.acres').css('display','none');
  	$('landInput').placeholder = 'Total Hectares...';
	} else {
		$('.acres').css('display','block');
		$('.hectares').css('display','none');
    $('landInput').placeholder = 'Total Acres...';
  }
});
$(window).on('load', function(){
  if (units.value === 'Hectares') {
		$('.hectares').css('display','block');
    $('.acres').css('display','none');
    document.getElementById('landInput').placeholder = 'Total Hectares...';
  } else {
		$('.acres').css('display','block');
    $('.hectares').css('display','none');
    document.getElementById('landInput').placeholder = 'Total Acres...';
  }
});
</script>
...
const selectInput = document.querySelector('#cropInput');
const crops = document.querySelectorAll('.crop-name-value');
...
crops.forEach((el) => {
  ...
  const values = text.split(',');
	const cropVal = values[0];
	const increaseVal = values[1] / 100 + 1;
  
	option.text = cropVal;
	option.value = increaseVal;
